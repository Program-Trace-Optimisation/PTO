...
def generator(n_inputs, max_hidden, n_outputs):
    ...
    network = []
    hidden_layer = [
        [rnd.normalvariate(0, 1) for _ in range(n_inputs + 1)] for _ in range(n_hidden)
    ]  # +1 for bias term
    network.append(hidden_layer)
    output_layer = [
        [rnd.normalvariate(0, 1) for _ in range(n_hidden + 1)] for _ in range(n_outputs)
    ]  # +1 for bias term
...
def forward_propagate_base_python(network, inputs):
    for layer in network:
        ...
        for neuron in layer:
            ...
            activation += sum(
                weight * input for weight, input in zip(neuron[:-1], inputs)
            )
...
def fitness_base_python(network, X_train, y_train):
    ...
    mse = np.mean(
        [
            sum((pred_i - target_i) ** 2 for pred_i, target_i in zip(pred, target))
