...
def make_training_data(n_samples, n_vars):
    ...
    target = lambda x: sum(coef[i] * (x[i]**i * x[(i+1) % n_vars] * x[(i+2) % n_vars]) for i in range(n_vars)) # target function
    X = np.array([[2*random.random() for _ in range(n_vars)] for _ in range(n_samples)]) # training inputs
...
def generator(grammar, max_depth=10): 
    ...
    def expand(left_sym, depth): 
        if left_sym in grammar:
            if depth <= 1:
                rules = [rule for rule in grammar[left_sym] if '<expr>' not in rule] # not recursive
            else:
                rules = grammar[left_sym]
            selected_index = rnd.choice(range(len(rules))) #rnd.randint(0, len(rules)-1)
            return ''.join([expand(right_sym, depth-1) for i, right_sym in enumerate(rules[selected_index])]) 
        else:
            return left_sym
...
if __name__ == '__main__':
    ...
    (pheno, geno), fx = run(generator, fitness, gen_args=(grammar,), 
