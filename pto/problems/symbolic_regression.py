...
def make_training_data(n_samples, n_vars, func_set, term_set):
    ...
    X = [[random.choice([False, True]) for _ in range(n_vars)] for _ in range(n_samples)] # training inputs
...
def full(func_set, term_set): # unused
    ...
    def _full(depth):
        if depth >= max_depth:
            expr = random.choice(term_set) # use random, not rnd, as this is for creation of problem instance
        else:
...
def cnf_generator(s):
    ...
    assert s % 3 == 0
    conjs = (f'(x[{i}] and x[{i+1}] and not x[{i+2}])' 
