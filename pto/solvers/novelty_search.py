...
class novelty_search:
    ...
    def __call__(self):
        ...
        local_comp_scores = self.compute_local_competition_all(
        ...
        for gen in range(self.n_generation):
            ...
            if self.verbose:
                print(
                    f"Gen {gen+1} - Best Fitness: {best[1]}, "
                    f"Avg Novelty: {sum(novelty_scores)/len(novelty_scores):.2f}, "
                    f"Avg Local Comp: {sum(local_comp_scores)/len(local_comp_scores):.2f}"
                )
            if self.return_history:
    ...
    def compute_novelty(self, individual, population):
        ...
        distances = [
            self.op.behavior_distance_ind(individual, other)
    ...
    def compute_local_competition(
        self, individual, index, population, fitness_population
    ):
        """Compute local competition score for an individual."""
        all_individuals = population + self.archive
        all_fitness = fitness_population + [
            self.op.evaluate_ind(ind) for ind in self.archive
        ]
        ...
        neighbors_indices = [
            i
            for i, other in enumerate(all_individuals)
            if i != index and self.op.behavior_distance_ind(individual, other) < radius
        ]
    ...
    def compute_local_competition_all(self, population, fitness_population):
        ...
        return [
            self.compute_local_competition(ind, i, population, fitness_population)
    ...
    def select_pop(self, population, novelty_scores, local_comp_scores):
        ...
        combined_scores = [
            self.selection_pressure * n + (1 - self.selection_pressure) * l
            for n, l in zip(novelty_scores, local_comp_scores)
        ]

        # Sort population by combined scores
        sorted_population = sorted(
            zip(population, combined_scores), key=lambda x: x[1], reverse=True
        )
...
if __name__ == "__main__":
    ...
    nslc = novelty_search(
        op=op,
        behavior_distance=op.behavior_distance_ind,
        population_size=50,
        n_generation=100,
    )
