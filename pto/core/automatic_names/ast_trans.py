...
class IterWrap(ast.NodeTransformer):
    ...
    def visit_comprehension(self, node):
        self.generic_visit(node)
        node.iter = ast.Call(
            func=ast.Name(id="iter_name", ctx=ast.Load()), args=[node.iter], keywords=[]
        )
        return node
...
class LoopCxt(ast.NodeTransformer):
    ...
    def visit_Loop(self, node):
        ...
        node.body = node.body + [
            ast.Expr(
                value=ast.Call(
                    func=ast.Name(id="count", ctx=ast.Load()), args=[], keywords=[]
                )
            )
        ]
        node = ast.With(
            items=[
                ast.withitem(
                    context_expr=ast.Call(
                        func=ast.Name(id="Loop_name", ctx=ast.Load()),
                        args=[],
                        keywords=[],
                    ),
